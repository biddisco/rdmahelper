...
public:
    ...
    struct rdma_event_channel *getEventChannel(void) const { return _eventChannel; }
...
//    struct ibv_context *getEventContext(void) const { return _event->id->verbs; }
        ...
        ssize_t length)
    {
        ...
        LOG_TRACE_MSG(_tag.c_str() << "posting Read wr_id " << hexpointer(send_wr.wr_id)
        ...
        ssize_t length, int flags)
    {
        ...
        ++_totalReadPosted;
        int err = ibv_post_send(_cmId->qp, &send_wr, &badRequest);
        LOG_TRACE_MSG(_tag.c_str() << "posting Write wr_id " << hexpointer(send_wr.wr_id)
            << " with Length " << hexlength(length) << " "
            << hexpointer(remoteAddr));
        return err;
    }
